...
- `expense-from-groups` opens add expense from `Your groups` without opening the group first. One group skips the picker. Several groups use a sheet. No groups offers `Create group`.
...
- On `Your groups`, choose `Add expense`. One group opens the form. Several groups ask you to pick one. No groups offers `Create group`.
