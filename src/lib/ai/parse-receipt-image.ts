...
export const receiptDraftJsonSchema = {
  ...
  properties: {
    ...
    amount: {
      type: "number",
      description:
        "Grand / receipt total as a positive decimal (not cents). Includes tax and tip.",
    },
    spentAt: {
      type: ["string", "null"],
...
- amount must be the grand / printed total (includes tax + tip when present).
...
- Tax and tip are implied by grand total minus sum of (item amount × quantity); leave that gap alone.
